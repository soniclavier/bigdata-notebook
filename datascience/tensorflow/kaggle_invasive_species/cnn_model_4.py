...
train_gen = train_data_gen.flow_from_directory('training_set_2', target_size=(128, 128), batch_size=25, class_mode='binary')
valid_gen = valid_data_gen.flow_from_directory('validation_set_2', target_size=(128, 128), batch_size=25, class_mode='binary')

#classification.load_weights("cnn_model_3.h5")
classification.fit_generator(train_gen, samples_per_epoch=1295, validation_data=valid_gen, nb_epoch=20, nb_val_samples=1000)

classification_json = classification.to_json()
with open("cnn_model_4.json", "w") as json_file:
    json_file.write(classification_json)

classification.save_weights("cnn_model_4.h5")
#70 val accuracy

valid_preds = classification.predict_generator(valid_gen, 1000)
...
with open("submission4.csv", "w") as output:
